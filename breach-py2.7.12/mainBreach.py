...
def isPlayerNearItemDrop(playerObject, itemDropObjectList, radiusToPlayerCenter, 
   ...
   '''     SYNTAX   isPlayerNearItemDrop(playerObject : obj, itemDropObjectList 
       ...
       PARAMETERS   The player object; the item drop object list; and the radius
           ...
           RETURN   True if the Player object is near an ItemDrop object.
                    Otherwise, False.
   '''
   return checkCircCollide( \
      (playerObject.rect.centerx, playerObject.rect.centery), playerObject, \
      itemDropObjectList, radiusToPlayerCenter + RADIUS_TO_ITEM_DROP_CENTER)
...
def getAllGunStats(EXPECTED_FPS = 30):
   ...
   stats_Gun_Mach = (30, int(EXPECTED_FPS / 8.0), 1, 60, 86, 101)
   stats_Gun_Shot = (50, int(EXPECTED_FPS * 1.5), 6, 50, 84, 86)
   stats_Gun_Aslt = (60, int(EXPECTED_FPS / 5.0), 1, 60, 86, 101)
   stats_Gun_Snip = (600, int(EXPECTED_FPS * 2.0), 1, 80, 90, 129)
   stats_Gun_Rail = (400, int(EXPECTED_FPS * 1.5), 16, 80, 82, 111)
...
def getAllUtilityNames():
   ...
   '''
   allUtilities = ("Util_Stunner", "Util_Barrier", "Util_Bomb")
...
def executeEntityShoot(isFromPlayer, entityObject, screenObject, \
                       spritesSolidGroup, RANDOM_BOT_SHOOT_INTERVAL = 3, \
                       IMPAIR_BOT_VELOCITY_FACTOR = 1.5, IMPAIR_BOT_DAMAGE_FACTOR = \
                       5.0, ACCEPTABLE_RADIUS_TO_TERRAIN_CENTER = 48):
...
def genCrateItem():
   ...
   if 1 <= pickedItemSection <= 60:
      # Picked Item: boost
      crateItem = getAllBoostNames()[random.randrange(0, len(getAllBoostNames()))]
   elif 61 <= pickedItemSection <= 85:
      # Picked Item: gun
      crateItem = getAllGunNames()[random.randrange(1, len(getAllGunNames()) - 1)]   
   elif 86 <= pickedItemSection <= 100:
      # Picked Item: utility
      crateItem = getAllUtilityNames()[random.randrange(0, len(getAllUtilityNames()))]
...
def checkForBrokenCrate(cratesObjectGroup):
   '''     SYNTAX   checkForBrokenCrate(cratesObjectGroup : group)
   
          PURPOSE   Check for a broken crate that exists in the world.
                    
       PARAMETERS   A group of all Crate objects.
       
           RETURN   The specific crate that was destroyed if it exists, 
                    otherwise, False.
   '''
...
def executeUtilityForm(utilityObject, midLayerPrecedenceList, botsObjectGroup, \
                       killableObjectsGroup, EFFECT_RADIUS = 256, \
                       BOMB_DAMAGE_ON_PLAYER_FACTOR = 0.40):
...
def spawnBotsByWave(radiusToPlayerCenter, screenObject, wavecounterObject, \
   ...
   if wavecounterObject.getWaveNumber() < wavecounterObject.getEndWaveNumber() - 1:
      ...
      botWaveHealth = int(round(wavecounterObject.getWaveNumber() ** 2) + 200)
      botWaveSpeed = 8 + wavecounterObject.getWaveNumber() / 3.0
      botCount = 3 + wavecounterObject.getWaveNumber() ** 2 / 90
      botADMultiplier = 0.50 + wavecounterObject.getWaveNumber() ** 2 / 250.0
   ...
   # Wave Number: boss wave
      ...
      botWaveHealth = 1000 + 750 * wavecounterObject.getWaveNumber()
      botWaveSpeed = 3 + 0.20 * wavecounterObject.getWaveNumber()
      botADMultiplier = 1.00
...
def handlingMiscInteractEvents(radiusToPlayerCenter, screenObject, wavecounterObject, \
                               healthbarObject, playerObject, cameraObject, midLayerPrecedenceList, \
                               botsObjectGroup, cratesObjectGroup, killableObjectsGroup, \
                               killableTerrainObjectsGroup, spritesSolidGroup):
   ...
   destroyedCrate = checkForBrokenCrate(cratesObjectGroup)
   if destroyedCrate:
...
def handlingMenuKeyPress(continueLoop):
...
def mainGameObjects(backgroundObject, worldSizeBorder, radiusToPlayerCenter, \
                    ...
                    INITIAL_WAVE_TIME = 450, INITIAL_PLAYER_HOTBAR_SLOT = 3, \
                    LAST_WAVE_NUMBER = 25, INITIAL_PLAYER_AD_MULTIPLIER = 1.00, \
                    ENDING_TEXT_COLOR = (255, 250, 250)):
   ...
   defaultItemDrop = classBreach.ItemDrop(getAllGunNames()[0], player.rect.center, True)
   
   # Create: Terrain
   barrels = []
   for barrelCount in xrange(8 ** 2):
   ...
   # Create: ending text
   displayFont = pygame.font.Font("fonts\\pixelated_arial_bold_11.ttf", 216)
   endingText = displayFont.render("GAME", 0, ENDING_TEXT_COLOR)   
   ...
   spritesAllTop = pygame.sprite.LayeredUpdates(player, bushes, spritesGUI)
...
def checkCircCollide((collidedPointX, collidedPointY), collidedObject, \
                     collidedCircGroup, groupCircRadius):
   '''  SYNTAX   checkCircCollide((collidedPointX : int, collidePointY : int), 
                 collidedObject, collidedCircGroup : Group, groupCircRadius : float)
   ...
   for collidedEntity in pygame.sprite.spritecollide(collidedObject, collidedCircGroup, False):
      # Test: if the player's arm is within the entity's body radius
         if math.sqrt((collidedPointX - collidedEntity.rect.centerx) ** 2 + \
                      (collidedPointY - collidedEntity.rect.centery) ** 2) <= groupCircRadius:
            # Return: values (case 1)
...
def handlingInput(keepGoing, screenObject, playerObject, worldSizeBorder, \
      ...
      if exitGameLoopCondition1:
         ...
         newBotsAppend1, newInGameItemDrop2, \
            ...
            handlingMiscInteractEvents(radiusToPlayerCenter, screenObject, wavecounterObject, \
                                       ...
                                       midLayerPrecedenceList, botsObjectGroup, \
      ...
      # Return: values
      return keepGoing, spritesInGameGroup, midLayerPrecedenceList, spritesAll
...
def executeMenu(screen, FRAMES_PER_SECOND = 30):
   ...
   while keepGoing:
      ...
      # E: EVENT HANDLING
      keepGoing = handlingMenuKeyPress(keepGoing)
...
def executeGameLoop(screen, ENDING_TIME = 2500):
   ...
   while keepGoing:
      ...
      # E: EVENT HANDLING
      keepGoing, spritesInGame, midLayerPrecedenceList, spritesAll = \
         handlingInput(keepGoing, screen, player, WORLD_SIZE_BORDER, RADIUS_TO_PLAYER_CENTER, \
                       wavecounter, healthbar, hotbar, camera, bots, spritesCrates, spritesKillable, \
                       spritesKillableTerrain, spritesSolid, spritesInGame, spritesAllBottom, \
                       midLayerPrecedenceList, spritesAllTop, spritesAll)
   ...
   # Execute: @ end of game
   screen.blit(endingText, (screen.get_width() / 2 - endingText.get_width() / 2, \
                      screen.get_height() / 2 - endingText.get_height() / 2))
   pygame.display.flip()
   pygame.time.wait(ENDING_TIME)
