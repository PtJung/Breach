...
class Utility(pygame.sprite.Sprite):
    ...
    def __init__(self, initialCoords, direction, utilityName, \
        ...
        self.__liveTime = 0
        self.__rotatedAngle = 0
        self.__throwSpeed = INITIAL_THROW_SPEED
        self.__changedForm = False
        
        filePath = "images\\entity\\"
        self.__imageAOECircle = pygame.image.load(filePath + "image_AOESignal.png").convert()
    ...
    def getRotatedAngle(self):
        '''  SYNTAX   object.getRotatedAngle()
    
            PURPOSE   Accessor Method: to get the object's rotated angle.
        
             RETURN   The object's rotated angle as an integer or a float value.
        '''
        return self.__rotatedAngle
    
    def addRotationToAngle(self, addedRotationAngle):
        '''     SYNTAX   object.addRotationAngle(addedRotationAngle : float)
        
               PURPOSE   To add a rotation angle to the existing sum of rotation
                         angles.
                         
            PARAMETERS   A float value of the angle amount to be added to the
                         existing sum of rotation angles.
        '''
        self.__rotatedAngle += addedRotationAngle
    ...
    def updateImage(self, initialCoords, AOE_DIAMETER = 512, IMAGE_SIDE_LENGTH = 64):
        ...
        imageUtility = pygame.image.load( \
            filePath + "image_" + self.__utilityName + ".png").convert()
        imageUtility = pygame.transform.scale(imageUtility, (IMAGE_SIDE_LENGTH, IMAGE_SIDE_LENGTH))
    ...
    def updateAOECircle(self, AOE_DIAMETER = 512):
        '''  SYNTAX   object.updateAOECircle()
        
            PURPOSE   To update the AOE signal radius.
        '''
        # Image and Rect: Place AOE Signal Diameter
        diameterOfCirc = int(round(self.getLiveTime() / float(self.__throwSpeed) * AOE_DIAMETER))
        if diameterOfCirc > AOE_DIAMETER:
            diameterOfCirc = AOE_DIAMETER
    ...
    def updateUtilitySpin(self):
        '''  SYNTAX   object.updateUtilitySpin()
             
            PURPOSE   To increase the step value of the object's spinning rate 
                      each frame until it reaches a point.
        '''
        # Image and Rect: rotate utility to face in its angle
        self.addRotationToAngle(self.__liveTime / 2)
        
    def utilitySpin(self):
        '''   SYNTAX   object.updateUtilitySpin()
             
             PURPOSE   To change the utility's spinning image while it exists.
        '''
        self.image = pygame.transform.rotate(self.__initImage, self.getRotatedAngle())
...
class WaveCounterGUI(pygame.sprite.Sprite):
    ...
    def update(self, STEP_FRAME_INCREMENT_PER_WAVE = 15):
        '''  SYNTAX   object.update()
...
class Entity(pygame.sprite.Sprite):
    '''     PURPOSE   Defines the sprite for the Entity, as a class.
        ...
        INHERITANCE   The Sprite class.
    '''
    def __init__(self, isPlayer, (initialCoordX, initialCoordY), inventory, \
                 selectBarSlot, health, attackDamageMultiplier, \
                 PLAYER_COLOR = (252, 200, 117), BOT_COLOR = (190, 200, 210)):
        '''     SYNTAX   object.Entity(isPlayer : bool, (initialCoordX : int, 
                         initialCoordY : int) : tuple, inventory : list, 
    ...
    def updateHeldLoc(self, (changeBaseX, changeBaseY)):
        '''     SYNTAX   object.updateHeldLoc((changeBaseX : int, changeBaseY : 
                         int) : tuple)
            ...
            PARAMETERS   The integer change in the base's X and Y coordinates, in 
                         two-index tuple.
        '''
        newGunX = self.__gunPos[0] + changeBaseX
        newGunY = self.__gunPos[1] + changeBaseY
        
        newUtilX = self.__utilPos[0] + changeBaseX
        newUtilY = self.__utilPos[1] + changeBaseY
        
        newBoostX = self.__boostPos[0] + changeBaseX
        newBoostY = self.__boostPos[1] + changeBaseY
        
        self.__gunPos = (newGunX, newGunY)
        self.__utilPos = (newUtilX, newUtilY)
        self.__boostPos = (newBoostX, newBoostY)
        
    def updateBaseLoc(self, (changeBaseX, changeBaseY)):
        '''     SYNTAX   object.updateBaseLoc((changeBaseX : int, changeBaseY : 
                         int) : tuple)
            ...
            PARAMETERS   The integer change in the base's X and Y coordinates, in 
                         two-index tuple.
        '''
        newBaseX = self.__relativeStartBase[0] + changeBaseX
        newBaseY = self.__relativeStartBase[1] + changeBaseY
...
class Player(Entity):
    ...
    def __init__(self, screen, inventory, selectBarSlot, health, attackDamageMultiplier, \
                 INITIAL_TOTAL_PUNCH_TIME = 4, INITIAL_WALK_SPEED = 10):
...
class Bot(Entity):
    ...
    def facePlayer(self, (playerCurrentX, playerCurrentY)):
        ...
        vector = (float(playerCurrentX - self.rect.centerx), \
                  float(playerCurrentY - self.rect.centery))
    ...
    def animateDeath(self, framesInDeath, FRAMES_UNTIL_STOP = 30, \
        ...
        self.updateArmLoc(False, changeInLArm, changeInRArm)
        self.equipHeldItemVisual()
...
class ItemDrop(pygame.sprite.Sprite):
    ...
    def update(self, EXPLODE_TIME = 50):
        ...
        # Expiration: after 900 frames (15 seconds)
